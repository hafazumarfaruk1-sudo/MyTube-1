...
import { View, Text, StyleSheet, Image, TouchableOpacity, FlatList, StatusBar, Dimensions, ActivityIndicator, DeviceEventEmitter } from 'react-native';
...
export default function ChannelScreen() {
    ...
    try {
      ...
      while (stack.length > 0) {
        ...
        if (node && typeof node === 'object') {
          let possibleTitle = node.title?.runs?.[0]?.text || node.title?.simpleText || node.headline?.runs?.[0]?.text || node.headline?.simpleText || node.title?.content || node.metadata?.lockupMetadataViewModel?.title?.content || node.overlayMetadata?.primaryText?.content; 
          if (possibleTitle && typeof possibleTitle === 'string') newTitle = possibleTitle;
        }
        ...
        if (Array.isArray(node)) {
          ...
          if (hasVideoId && !seenIds.has(node.videoId)) {
            ...
            if (!exactTitle || exactTitle === 'Unknown Title') exactTitle = 'Unknown Title'; 
            ...
            const thumbnailUrl = thumbQuality === 'Data Saver' ? `https://i.ytimg.com/vi/${vId}/mqdefault.jpg` : `https://i.ytimg.com/vi/${vId}/hqdefault.jpg`;
            const videoUrl = tabType === 'Shorts' ? `https://www.youtube.com/shorts/${vId}` : `https://www.youtube.com/watch?v=${vId}`;

            categorizedData[tabType].unshift({
              id: String(vId), title: String(exactTitle), value: videoUrl, channel: channelName, avatar: channelAvatar, 
              duration: duration || (tabType === 'Shorts' ? 'Short' : ''), publishedTime: publishedTime || (isLive ? 'Live Now' : ''),
              views: views, thumbnail: thumbnailUrl, isLive: isLive
            });
          }
  ...
  const parseYtData = (html) => {
    try {
      let match = html.match(/ytInitialData\s*=\s*({.+?});/) || html.match(/var ytInitialData\s*=\s*(.*?);<\/script>/) || html.match(/window\["ytInitialData"\]\s*=\s*({.+?});/);
  ...
  const findBannerUrl = (data) => {
    ...
    try {
      const search = (obj) => {
        ...
        const bannerSources = obj.tvBanner?.thumbnails || obj.mobileBanner?.thumbnails || obj.banner?.thumbnails || obj.imageBannerViewModel?.image?.sources || obj.pageHeaderBannerImageViewModel?.image?.sources;
    ...
    try {
        const fetchTabViaApi = async (endpoint, tabName) => {
            ...
            const response = await fetch(`https://www.youtube.com/youtubei/v1/browse?key=${currentApiKey}`, {
                ...
                body: JSON.stringify({ context: { client: { clientName: 'WEB', clientVersion: '2.20231214.00.00' } }, browseId: endpoint.browseId, params: endpoint.params })
            });
        ...
        const [apiVideos, apiShorts] = await Promise.all([ fetchTabViaApi(vEndpoint, 'Videos'), fetchTabViaApi(sEndpoint, 'Shorts') ]);
  ...
  const fetchChannelData = async () => {
    ...
    try {
      ...
      if (!extractedChannelUrl) {
          ...
          if (searchData) {
            const findChannelUrl = (node) => {
              ...
              if (node?.channelRenderer?.navigationEndpoint?.commandMetadata?.webCommandMetadata?.url) { extractedChannelUrl = node.channelRenderer.navigationEndpoint.commandMetadata.webCommandMetadata.url; return; }
              if (node?.videoRenderer?.ownerText?.runs?.[0]?.navigationEndpoint?.commandMetadata?.webCommandMetadata?.url) { extractedChannelUrl = node.videoRenderer.ownerText.runs[0].navigationEndpoint.commandMetadata.webCommandMetadata.url; return; }
      ...
      const [videosRes, shortsRes] = await Promise.all([ fetch(targetVideosUrl, { headers: { 'User-Agent': DESKTOP_AGENT } }), fetch(targetShortsUrl, { headers: { 'User-Agent': DESKTOP_AGENT } }) ]);
      ...
      if (extractedBanner) setChannelBanner(extractedBanner);
      ...
      if (currentApiKey) {
          let vEndpoint = null; let sEndpoint = null;
          const extractEndpoints = (data) => {
              ...
              tabs.forEach(t => {
                  ...
                  if (title.includes('video') || title.includes('ভিডিও')) vEndpoint = vEndpoint || t?.tabRenderer?.endpoint?.browseEndpoint;
                  if (title.includes('short') || title.includes('শর্ট')) sEndpoint = sEndpoint || t?.tabRenderer?.endpoint?.browseEndpoint;
              });
          };
  ...
  const handleShortPress = (item, index) => {
    ...
    navigation.navigate('Shorts', { 
        videoId: item.id, url: item.value, title: item.title, videoData: videoInfo,
        channelAvatar: channelAvatar, shortsList: tabData.Shorts, initialIndex: index 
    });
  };
    ...
    <View>
      <View style={styles.bannerContainer}>
        <Image 
            ...
            style={[styles.bannerImage, { position: 'absolute' }]} blurRadius={15} 
        />
        {channelBanner ? (
            <Image 
                source={{ uri: channelBanner }} 
                style={[styles.bannerImage, { opacity: isBannerLoaded ? 1 : 0 }]} 
                onLoad={() => setIsBannerLoaded(true)} onError={() => setIsBannerLoaded(true)} 
            />
        ) : null}
      </View>

      <View style={styles.channelProfileSection}>
        <TouchableOpacity style={styles.avatarWrapper} activeOpacity={isLiveChannel ? 0.7 : 1} onPress={() => {}}>
      ...
      <View style={styles.tabScrollContainer}>
        <FlatList 
          horizontal={true} showsHorizontalScrollIndicator={false} data={['Videos', 'Shorts']} 
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: isDarkMode ? '#0F0F0F' : '#F9F9F9' }]}>
      ...
      <FlatList 
        ...
        ListHeaderComponent={ChannelHeader} ListEmptyComponent={renderEmptyComponent} ListFooterComponent={renderFooter}
        onEndReached={fetchMoreData} onEndReachedThreshold={0.5} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 80 }} 
      />
    </SafeAreaView>
  );
}
...
function getDynamicStyles(isDark) {
  return StyleSheet.create({
    ...
    shortsColumnWrapper: { justifyContent: 'flex-start', paddingHorizontal: 2, marginTop: 10 },
    shortCard: { width: (width / 3) - 4, marginHorizontal: 2, marginBottom: 4, backgroundColor: isDark ? '#0F0F0F' : '#FFFFFF' },
    shortThumbnailWrapper: { width: '100%', height: ((width / 3) - 4) * 1.77, borderRadius: 8, overflow: 'hidden', backgroundColor: isDark ? '#222' : '#ddd' },
    shortThumbnail: { width: '100%', height: '100%', resizeMode: 'cover' },
